...
export function proxy(request: NextRequest) {
  ...
  if (
    ...
    ((request.nextUrl.pathname === '/api/settings' || request.nextUrl.pathname === '/api/certifications') && request.method !== 'GET')
  ) {
    return NextResponse.json({ error: 'Authentication required' }, { status: 401 })
  }

  if (
    (request.nextUrl.pathname === '/api/settings' || request.nextUrl.pathname === '/api/certifications') &&
    request.method === 'GET'
  ) {
...
export const config = {
  matcher: ['/UI-Components/Admin', '/api/upload', '/api/settings', '/api/certifications'],
}